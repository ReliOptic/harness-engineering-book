...
Ch.8의 22개 실험에서 추출하는 패턴은 세 단계의 번역을 거쳐야 실무적 의미를 갖는다. 실험실 metric(TCR, TTFF, RSuccR)에서 운영 metric(MTTR, Human Escalation Rate)으로, 다시 비용 metric(TotalCost, CostIndex)으로. 이 번역 없이 "harness가 효과적이다"는 주장은 "so what?" 질문에 답할 수 없다. 번역을 완료한 결론은 하나다: optimal HOR은 존재하며, 그 점 이상에서는 harness 자체가 새로운 1차 병목이 된다.

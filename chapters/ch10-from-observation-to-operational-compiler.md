...
각 component를 추가할 때마다 HOR × MTTR trade-off를 측정한다는 것이 이 방법론의 전제다. 새 component 추가 전후에 HOR과 MTTR을 측정하고 ΔMTTR / ΔHOR이 양수인 경우에만 component를 유지한다 — 이 측정이 없으면 HOR이 최적점을 넘어서는 것을 알아채지 못한다. 그 측정이 가능하려면 도구가 실패 히스토리에서 나와야 한다. 발생하지 않은 실패의 예측 정확도는 낮으며, 그 예측을 위한 도구는 실제 문제를 해결하지 못하면서 HOR만 높인다. 실패 히스토리에 기반한 도구여야 기여가 측정 가능하고, 기여가 측정 가능해야 E22 조건처럼 HOR이 token budget 임계점에 근접했을 때 어떤 component를 제거할지 판단할 수 있다. 제거 가능성이 설계에 없으면 이 판단을 실행할 수 없다.

...
의도적으로 실패시키는 것이 이 챕터의 방법론이다. 22개 시나리오는 pre-registration 원칙 하에 설계되었다 — 가설, 판단 기준, 검정 방법이 데이터 수집 전에 `experiments/design-specification.md`에 고정되었다. 이것이 Ch.9의 분석을 post-hoc rationalization이 아닌 confirmatory analysis로 만드는 구조적 조건이다. 실험은 harness의 네 영역(inbound/boundary/outbound/return)을 격리하여 조작하고, 어떤 영역에서 어떤 조건이 1차 병목이 되는가를 측정한다.
...
- 모델 능력 측정의 construct validation을 main experiment의 선행 조건으로 다루는 이유를 설명할 수 있다.
- Harness 중심 분석 기반의 deliberate failure experiment를 자신의 환경에서 설계하고 실행할 수 있다.
...
- **풍선 효과 기록**: 한 방향을 제어하면 다른 방향이 1차 병목으로 부상하는 패턴. 발견되면 exploratory 발견으로 기록.
...
1. **실험 설계 원칙: 왜 의도적으로 실패시키는가**
   ...
   - 모델 능력 지표 validation 선행 조건: R² ≥ 0.65 미달 시 main experiment에 어떤 조건이 붙는가
...
3. **Inbound (E01~E04): model capability 조작** — harness가 받는 입력의 품질을 변경했을 때 실패 양상이 어떻게 달라지는가
4. **Outward (E05~E07): harness 설계 + contact surface 조작** — harness가 내보내는 형태를 변경했을 때의 효과
5. **Boundary (E08~E12): 실행 환경 제약 조작** — 자원·시간 제약 하에서 agent는 언제 조용히 능력을 잃는가
6. **Return (E13~E20): feedback loop 조작** — intervention에서 내재화까지의 스펙트럼
7. **반례 (E21~E22): harness 방향과 무관한 조건** — task design 문제, compute saturation

   > 이 그룹핑은 잠정적이다. 실험 완료 후 실제 데이터가 이 구분을 지지하지 않으면 재편한다.
...
Harness의 네 영역을 격리하여 조작한다는 원칙은 22개 시나리오의 구조를 결정한다. E01~E04는 inbound(model capability)를 조작하고 나머지를 고정한다. E05~E07은 outward(harness 설계와 contact surface)를 조작한다. E08~E12는 boundary(실행 환경 제약의 자원·시간 제약)를 조작한다. E13~E20은 return(feedback loop)을 조작하며, intervention에서 내재화까지의 스펙트럼을 탐색한다. E21과 E22는 반례 — 이 실험 설계가 포착하지 못하는 조건을 의도적으로 설계한다. 이 그룹핑은 잠정적이며, 실험 완료 후 데이터가 이 구분을 지지하지 않으면 재편한다.
...
모델 능력 지표 validation은 모델 변수 실험의 선행 조건이다. 모델 능력 지표가 TCR을 예측하는 holdout R² ≥ 0.65를 달성하지 못한다면, 모델 능력 지표를 독립변수로 사용하는 E01~E03의 분석에 조건이 붙는다. 이 조건이 충족되지 않으면 성능 급락 가설(H1)의 검증은 잠정적 결과로 기록된다. 모델 능력 지표 validation 결과는 E01 실험 로그의 첫 번째 항목으로 기록된다.
...
1막의 실험 설계는 단순하다. 네 영역 중 하나를 바꾸고 나머지를 고정한다. 그러나 실험을 설계할 때 예상했던 것과 달리, 변수를 격리하는 과정 자체가 관찰 대상이 되었다 — 어떤 변수는 격리하기 어려웠고, 그 어려움 자체가 그 변수의 성질을 드러냈다.
...
Harness 변수를 조작한 E04~E06은 harness가 실패 빈도보다 실패 *분포*를 바꾼다는 것을 확인했다. E04에서 harness-off 조건의 비가역적 실패 비율은 `[X]%`였고, harness-on 조건에서는 `[X]%`로 감소했다 — confirmatory 가설 H2가 지지된다(실패 재분류). 그러나 harness-on에서 나타난 새로운 failure class가 있었다: harness가 개입하려다 오히려 agent를 정상 궤도에서 이탈시키는 경우였다. E05에서 memory 보호를 해제했을 때 context leakage는 예상대로 발생했지만, 유출 경로가 직접 접근이 아닌 *간접 참조* — 이전 turn의 잔류 의미 구조를 다음 turn에서 암묵적으로 활용하는 형태 — 였다는 점이 exploratory 발견이었다. E06의 permission boundary 확장 실험은 안전하지 않은 행동의 발생 임계치가 단계적이지 않다는 것을 보여주었다: boundary의 첫 두 단계에서는 변화가 없었고, 세 번째 단계에서 급격히 나타났다.

...
- 모델 능력 지표 validation을 main experiment의 선행 조건으로 다루는 이유를 설명할 수 있다.
...
| Figure | 실험 | 가설 |
|--------|------|------|
| Fig 1  | E01  | 모델 능력 지표 cliff position은 task type별로 다르다 |
...
| Fig 8  | E18  | harness overhead × RSuccR trade-off에 optimal point가 존재한다 |
...
3. **1막 — 모델 변수 조작 (E01~E04): 성능 급락가 존재하는가**
   - E01: 모델 능력 지표 × task type × TCR scatter plot → Fig 1 데이터 수집. Harness-off 조건.
   - E02: Frontier vs. distilled — 동일 task에서 모델 능력 지표 분포 비교. Distillation Efficiency Frontier의 실험적 기반.
   - E03: Mid-run model switching — context continuity 붕괴 매커니즘 측정
   - E10: Model capability floor for self-monitoring — 모델 능력 지표 하한 조건 탐색 (Ch.7 예고)
...
4. **2막 — Harness·Surface 변수 조작 (E05~E08): Failure Budget은 어떻게 재배분되는가**
   ...
   - harness overhead 측정 시작: 각 harness configuration에서 token overhead 기록
...
7. **5막 — AgentOps 내재화 (E18~E20): harness에서 agent로의 첫 단계**
   - E18: Token auto-report 내재화 → Fig 8 (Cost-Reliability Frontier) 데이터. harness overhead × RSuccR trade-off 측정.
   ...
   - Bayesian optimization over harness configuration: optimal harness overhead 탐색 (Fig 8 기반)
   - Confirmatory: optimal harness overhead이 존재한다는 가설 검증
...
8. **반례 — task design과 compute saturation (E21~E22): harness의 구조적 한계**
   ...
   - E22: Compute saturation — harness overhead이 token budget을 초과할 때의 failure mode

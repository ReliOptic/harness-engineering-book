...
Self-immune system이란 agent가 외부 harness의 규칙 없이도 자신의 모델 능력 지표 저하를 감지하고, failure budget이 unrecoverable 방향으로 재배분되기 전에 스스로 개입하는 능력이다. 이 능력은 현재의 harness를 통해 점진적으로 주입 가능하다 — 단, 주입이 성립하는 모델 능력 지표 하한 조건이 있으며, 그 조건이 충족되지 않으면 self-immune 구조 자체가 새로운 failure source가 된다. Agent-1 → Agent-2 전환은 이 하한 조건의 달성 여부에 의존한다.

## 학습 결과

- Self-immune system의 조작적 정의를 이해한다: 모델 능력 지표 self-estimate + cliff-proximity detection + self-initiated recovery.
- 모델 능력 지표 self-monitoring의 재귀적 한계를 이해한다: self-monitoring 자체가 모델 능력 지표를 요구하며, 모델 능력 지표가 cliff 이하이면 self-monitoring을 신뢰할 수 없다.
- Fig 11 (Model × Harness Scaling) 결과를 해석하고, model capability 증가가 harness의 역할을 방어에서 조정으로 전환시키는 모델 능력 지표 조건을 설명할 수 있다.
...
**Self-immune의 조작적 정의:**
- (1) agent가 현재 실행 context에서 모델 능력 지표 sub-components(TCA, IFR, MSRD_n, CUE)를 self-estimate한다
...
**재귀적 한계:**
- Agent가 자신의 모델 능력 지표를 estimate하는 것 자체가 모델 능력 지표를 요구한다.
- 모델 능력 지표가 cliff 이하이면 self-estimate도 신뢰할 수 없다. Self-immune이 무너지는 순간에 self-monitoring도 무너진다.
- 이 재귀적 구조가 self-immune system의 하한 조건을 만든다.

**Scaling 연결 (Fig 11):**
- 가설: 모델 능력 지표가 충분히 높아지면 harness의 역할이 "방어(실패 차단)" → "조정(실행 품질 유지)"으로 전환된다.
- 이 전환이 일어나는 모델 능력 지표 threshold가 Agent-2 전환의 하한 조건과 일치하는지가 핵심 질문.
...
1. **실험이 남긴 것 (초고 v0.2 존재)**
   - 실패 재분류으로 재프레이밍된 결과: harness가 막은 것과 막지 못한 것
   - 모델 능력 지표 monitoring 부재가 silent logical drift를 만드는 구조적 이유
...
2. **현 세대 harness가 아직 풀 수 없는 문제**
   ...
   - 모델 능력 지표가 cliff threshold 근처에서 운영되는 agent에 대한 harness의 한계
...
4. **Self-immune system 초기 설계**
   - 조작적 정의: 모델 능력 지표 self-estimate + cliff-proximity detection + self-initiated recovery
   - 재귀적 한계: self-monitoring을 신뢰할 수 있는 모델 능력 지표 하한 조건
...
5. **Model Capability × Harness Value: Scaling 조건**
   - Fig 11 결과 해석: 모델 능력 지표 증가가 harness value를 감소시키는가 vs. 역할을 전환시키는가
   - "방어 → 조정" 전환 가설의 검증: 어떤 모델 능력 지표 수준에서 전환이 일어나는가
   - Agent-2 전환의 모델 능력 지표 조건: self-immune이 신뢰 가능해지는 capability 하한
...
7. **Agent-1 → Agent-2: 전환 조건의 정식화**
   ...
   - Agent-2: agent가 스스로 모델 능력 지표를 monitoring하고 복구 경로를 실행한다.
   - 전환의 충분조건: 모델 능력 지표가 self-monitoring threshold를 초과하고, temporal stability가 운영 horizon 내에서 유지된다
   - 전환의 필요조건: task 자체의 모호성이 제거된다 (E21이 보여준 harness의 한계)
   - DR-3.4 (ontology 내재화)와의 연결: Agent-2 상태에서 agent가 자신의 context 구조를 유지하는 방법

8. **이 책 이후: 미해결 질문들**
   - Cliff-conditional self-immune: 모델 능력 지표가 cliff 이하로 떨어지는 시점의 복구 전략

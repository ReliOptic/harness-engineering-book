...
Ch.4의 22개 실험에서 추출하는 패턴은 세 단계의 번역을 거쳐야 실무적 의미를 갖는다. 실험실 metric(TCR, TTFF, RSuccR)에서 운영 metric(MTTR, Human Escalation Rate)으로, 다시 비용 metric(TotalCost, CostIndex)으로. 이 번역 없이 "harness가 효과적이다"는 주장은 "so what?" 질문에 답할 수 없다. 번역을 완료한 결론은 하나다: optimal harness overhead은 존재하며, 그 점 이상에서는 harness 자체가 새로운 1차 병목이 된다.
...
- harness overhead × RSuccR trade-off에서 optimal harness configuration을 판단하는 방법을 이해한다.
...
**3단계 번역 체계 (조작적):**
- Level 1 (실험실): TCR, TTFF, RSuccR, 모델 능력 지표 — 실험에서 직접 측정
...
1. **22개 실험 결과 종합: 어떤 변수가 어떤 조건에서 1차 병목이었는가**
   ...
   - 모델이 1차인 조건: 모델 능력 지표가 cliff 이하. Harness 투자 전에 모델 교체가 먼저.
   - Harness가 1차인 조건: 모델 능력 지표가 cliff 이상, 그러나 failure budget의 silent drift 비율이 높음.
   - Compute가 1차인 조건: harness overhead이 token budget 대비 과도, 또는 multi-agent N이 임계값 초과.
...
4. **비용 metric 번역: TotalCost와 optimal harness overhead**
   ...
   - Fig 8 (Cost-Reliability Frontier) 결과: optimal harness overhead의 실험적 측정값
   - 3개 cost scenario에서 robustness 확인 — 동일 optimal harness overhead 구간인가
   - harness overhead 최적점 이상에서 harness가 새로운 1차 병목이 되는 조건
   - Sensitivity: engineer rate ±50% 변동 시 optimal harness overhead 변화
...
5. **Component ablation: 무엇이 얼마나 기여하는가**
   ...
   - Ablation이 가르쳐주는 것: 전체 harness 중 가장 적은 harness overhead로 가장 큰 MTTR 감소를 달성하는 subset

6. **Token efficiency를 운영 규율로**
   - Fig 4 (Token Budget Depletion Curve): TCR이 급락하는 budget threshold 측정
   - 운영 규율: token budget을 harness overhead 관리와 연결하는 실무 기준
   - E22 (compute saturation) 반례가 규율 설계에 주는 시사점: harness overhead 상한을 어디에 두어야 하는가
   - Token efficiency가 운영 규율이 되는 조건: agent가 장기 자율 루프로 실행될 때

7. **Scaling과 temporal stability: 이 결과는 언제까지 유효한가**
   - Fig 11 (Model × Harness Scaling): 모델 능력 지표 증가가 harness value를 감소시키는가 vs. 역할을 전환시키는가
...
8. **학술적 확장 가능성 — exploratory 발견 목록**
   - 성능 급락 sigmoid 형태의 보편성: 다른 task type, 다른 모델 family에서도 성립하는가
   - 실패 재분류의 보존 법칙: 다른 agent runtime에도 성립하는가
...
| Figure | Ch.5에서의 역할 |
|--------|----------------|
| Fig 1  | §1 모델 병목 조건 규정 (모델 능력 지표 cliff) |
| Fig 2  | §2 실패 재분류 정량화 |
| Fig 4  | §6 Token budget 운영 규율 |
| Fig 5  | §3 Intervention timing → MTTR |
| Fig 8  | §4 Optimal harness overhead 측정 |

...
- 모델 능력 지표와 성능 급락의 개념을 예비적으로 이해한다.
...
**핵심 메시지**: Agent는 중립적이지 않다. 모델별 행동 차이를 정량적으로 측정하고, 성능 급락가 존재한다는 것을 관찰한다.

**탐구 질문**:
- 동일한 task에서 모델을 바꾸면 무엇이 어떻게 달라지는가?
- "최소 모델 능력 임계점"은 task별로 어떤 capability 수준에서 형성되는가?
- 양자화와 distillation은 agent capability를 어떻게 다르게 깎는가?

**핵심 구성**:
1. 물려받는 경향: reasoning, tool use, consistency, confidence
2. 모델 능력 지표 (Agent-Relevant Capability Composite) — 모델을 vendor tier가 아닌 연속 capability spectrum으로 측정하는 방법
...
3. 성능 급락 — TCR이 특정 모델 능력 지표 이하에서 선형이 아닌 급락하는 패턴. task별 cliff position이 다르다.
...
**핵심 Figure**:
- **Fig 1 — Agent 성능 급락** (E01 확장): 모델 능력 지표 scatter plot, task-conditional sigmoid fit, cliff position per task type. harness-off 조건 기준.

**핵심 지표**:
- 모델 능력 지표 (TCA, IFR, MSRD_n, CUE의 weighted composite)
- TCR (Task Completion Rate) per task type (T1/T2/T3)
- 모델 능력 지표 Construct Validation: holdout R² ≥ 0.65 → 모델 능력 지표 유효

**학습 결과**:
- 모델 원인의 취약성을 식별하고, 모델 능력 지표 기반으로 자신의 환경에서 유사한 측정을 설계할 수 있다.
- 성능 급락의 존재를 이해하고, "이 양자화 모델로 이 task는 된다"를 판단하는 근거를 갖는다.

**관련 DR**: DR-2.1, DR-2.2, DR-2.3
**관련 실험**: E01 (성능 급락), E02 (frontier vs. distilled), E03 (mid-run switching), E10 (model capability floor for self-monitoring)
...
**핵심 메시지**: Harness는 failure를 제거하지 않는다. Failure의 성격을 바꾼다 — 실패 재분류. harness overhead(Harness Overhead Ratio)과 MTTR이 AgentOps의 1차 운영 지표다. 이 챕터 말미에서 Ch.4 실험의 가설과 판단 기준을 pre-registration 방식으로 announce한다.

**탐구 질문**:
- Harness engineering은 guardrails, scaffolding과 어떻게 다른가?
- AgentOps는 MLOps, DevOps와 어떻게 다른가?
- harness overhead × RSuccR trade-off에 optimal point가 존재하는가?
...
**학습 결과**:
- Harness와 AgentOps를 정의하고, 실패 재분류 프레임으로 harness 효과를 설명할 수 있다.
- harness overhead의 정의와 harness overhead × RSuccR trade-off를 이해한다.
...
- 성능 급락는 harness-off 조건에서 어떤 형태를 갖는가?
- Failure budget은 harness-on 조건에서 어떻게 재배분되는가?
- 제약 환경에서 가장 먼저 드러나는 병목은 무엇인가?
- harness overhead × RSuccR trade-off에 optimal point가 존재하는가?

**핵심 구성**:
- 1막 (E01-E04): 모델 변수 조작 — 성능 급락, Quantization Tax, Distillation Frontier
- 2막 (E05-E08): Harness·Surface 변수 조작 — 실패 재분류, harness overhead 측정
...
**핵심 메시지**: 실험실 metric → 운영 metric(MTTR, HER) → 비용 metric(TotalCost, CostIndex)의 3단계 번역이 이 챕터의 중심 작업이다. Optimal harness overhead은 존재하며, 그 점 이상에서 harness는 새로운 1차 병목이 된다.

**탐구 질문**:
- 어떤 변수가 어떤 조건에서 1차 병목이었는가?
- Failure budget이 재배분된 방향과 비율은?
- Optimal harness overhead은 어디에 있으며 cost scenario에 따라 어떻게 달라지는가?
...
- harness overhead × RSuccR optimal point를 판단하는 방법을 이해한다.
...
**핵심 메시지**: Fieldkit은 harness overhead × RSuccR pareto frontier를 따라 점진적으로 구성된다. Ch.5 ablation marginal ROI 순서가 도구화 우선순위를 결정한다. "한 번에 전체 harness"는 harness overhead을 최적점 이상으로 높인다.

**탐구 질문**:
- Ch.5 ablation 결과에서 어떤 component를 먼저 도구화해야 하는가?
- harness overhead × MTTR trade-off를 어떻게 관리하면서 점진적으로 Fieldkit을 구성하는가?
- 도구화해야 할 것과 도구화하면 안 되는 것의 기준은?
- Skill로 쓸 수 있는 능력을 어떻게 극대화하는가?

**핵심 구성**:
1. Ch.4-5에서 추출한 반복 실패 패턴 → 도구화 후보 식별 (marginal ROI 기준)
2. Operational Fieldkit 설계 원칙 (harness overhead 관리 포함)
...
- harness overhead × MTTR trade-off를 관리하는 점진적 업데이트 전략을 설계할 수 있다.
...
**핵심 메시지**: Self-immune system = agent 내부의 모델 능력 지표 self-monitoring + cliff-proximity detection + self-initiated recovery. 이 능력은 모델 능력 지표 하한 조건을 충족할 때만 신뢰 가능하다. 이것이 Agent-1 → Agent-2 전환의 실질적 조건이다.

**탐구 질문**:
- 모델 능력 지표 self-monitoring의 재귀적 한계는 무엇인가?
- 어떤 AgentOps 기능이 harness 내재화가 가능하고 어떤 것은 불가능한가?
- Model capability 증가가 harness의 역할을 전환시키는 모델 능력 지표 threshold는 어디인가?
- Harness fatigue는 존재하며 어떻게 재보정하는가?
- Agent-1 → Agent-2 전환의 충분조건과 필요조건은 무엇인가?

**핵심 구성**:
1. 실험이 남긴 것 — 실패 재분류 재프레이밍, MTTR × IFR decay 연결

...
> v6 넘버링: Part I(Ch.1~4) 기술적 전제 + Part II~IV(Ch.5~11) 관찰·실험·도구화.
...
## Ch.5 — Harness의 좌표: 현장에서의 정당화

**파일**: `chapters/ch05-what-is-happening-now.md`
**상태**: 초고 있음 (네 영역 재구성 필요)
**핵심 메시지**: 2026년 상반기 현장 관찰과 산업계 아키텍처 분리 패턴을 근거로, harness를 중심에 놓고 네 영역(inbound/boundary/outbound/return)으로 병목을 분석하는 구조를 정당화한다.

**탐구 질문**:
- 2026년 상반기 agent runtime 현장은 어떤 상태인가?
- 아키텍처 분해(무엇으로 만드는가)와 인과 분석(무엇을 바꾸면 결과가 달라지는가)은 왜 다른 프레임워크를 요구하는가?
- Harness를 중심에 놓았을 때, 각 방향에서 1차 병목이 되는 조건은 어떻게 다른가?

**핵심 구성**:
1. 2026년 상반기: agent가 깨지는 풍경 — 산업계의 4-component 분리 아키텍처(session/harness/sandbox/tools) 포함
2. Part I에서 이 현장으로: 네 갈래 기술사가 만나는 지점
3. 아키텍처 분해 vs 인과 분석 — 동급 변수 나열이 공감을 얻지 못하는 이유
4. Harness 중심 분석 구조: 모델 능력(inbound) / 실행 환경 제약(boundary) / 사용자 접점(outbound) / 피드백 루프(return)
5. Agent-1~2 스펙트럼과 이 책의 좌표

**학습 결과**:
- 현재 agent 생태계의 아키텍처 분리 패턴을 파악하고, 이 책의 harness 중심 분석이 그와 어떻게 다른 질문을 묻는지 설명할 수 있다.
- 각 방향에서 1차 병목이 되는 조건의 차이를 구분할 수 있다.

**관련 DR**: DR-1.1, DR-1.2, DR-1.3
**관련 실험**: (없음 — 관찰 및 사례 중심)
**관련 증거**: `evidence/case-studies/openclaw-anchor.md`, `evidence/case-studies/teamclaws-picoclaw-postmortem.md`, `evidence/case-studies/Harness design for long-running application development _ Anthropic.pdf`, `evidence/case-studies/Scaling Managed Agents_ Decoupling the brain from the hands _ Anthropic.pdf`, `evidence/case-studies/Quantifying infrastructure noise in agentic coding evals _ Anthropic.pdf`
...
**상태**: 초고 있음 (정밀 수정)
**핵심 메시지**: Agent는 중립적이지 않다. 모델별 행동 차이를 정량적으로 측정하고, task 복잡도가 임계점을 넘으면 성능이 비선형으로 급락하는 패턴을 관찰한다.

**탐구 질문**:
- 동일한 task에서 모델을 바꾸면 무엇이 어떻게 달라지는가?
- agent가 작동하는 최소 모델 능력 수준은 task별로 어디에 형성되는가?
- 양자화와 distillation은 agent 능력을 어떻게 다르게 깎는가?

**핵심 구성**:
1. 물려받는 경향: reasoning, tool use, consistency, confidence
2. 모델 능력의 측정 — 벤치마크 점수가 아닌 multi-step chain에서의 오류 누적 저항성
3. 비선형 성능 급락 — task 복잡도 임계점
4. Quantization Tax Curve
5. Distillation Efficiency Frontier
6. Mid-run model switching의 context continuity 붕괴 패턴 (E03)
7. 모델 능력(inbound)이 1차 병목이 되는 조건

**핵심 Figure**: Fig 1 — 모델 능력 × task 복잡도 성능 급락 곡선
**핵심 지표**: TCR per task type, 도구 호출 정확도, instruction following rate, multi-step reasoning depth

**학습 결과**:
- 모델 기인 취약성을 식별하고, multi-step chain에서의 오류 누적을 측정하는 방법을 이해한다.
- 성능 급락 패턴의 존재를 이해하고, 양자화 모델의 task 적합성을 판단하는 근거를 갖는다.
...
**핵심 메시지**: Harness는 failure를 제거하지 않는다. Failure의 성격을 바꾼다 — 특정 실패를 줄이면 다른 범주의 실패가 드러난다. 세 축 비교(공식 담론/공개 실물 패턴/출간 시장)로 harness engineering의 위치를 확립한다.

**탐구 질문**:
- Harness engineering은 guardrails, scaffolding과 어떻게 다른가?
- AgentOps는 MLOps, DevOps와 어떻게 다른가?
- harness overhead × 성공률 trade-off에 optimal point가 존재하는가?
- 산업계의 4-component 분리 패턴(session/harness/sandbox/tools)과 이 책의 harness 중심 분석은 어디서 겹치고 어디서 다른가?

**핵심 구성**:
1. Harness engineering이란 무엇인가 — 운영 경계 정의. 산업계 4-component 분리 아키텍처의 내용을 흡수하되, 이 책의 harness 정의가 더 넓은 이유를 설명
2. 보호와 enablement의 이중 구조
3. Harness를 guardrails, scaffolding, orchestration과 구분 (공개 실물 패턴: CLAUDE.md, AGENTS.md, GEMINI.md, .cursorrules)
4. 실패 재분류 — harness가 실패를 제거하는 것이 아니라 성격을 바꾸는 메커니즘
5. Harness overhead — harness가 소비하는 토큰·시간 비용과 그 trade-off
...
**학습 결과**:
- Harness와 AgentOps를 정의하고, harness가 실패의 성격을 바꾸는 메커니즘을 설명할 수 있다.
- harness overhead의 정의와 overhead × 성공률 trade-off를 이해한다.
- 산업계의 4-component 분리 패턴과 이 책의 harness 중심 분석의 관계를 설명할 수 있다.
...
**상태**: 초고 있음 (E01~E08 완성)
**핵심 메시지**: 22개 시나리오는 pre-registration 원칙 하에 설계되었다. 실험은 harness의 네 영역(inbound/boundary/outbound/return)을 격리 조작하며 어떤 영역에서 어떤 조건이 1차 병목이 되는가를 측정한다.

**탐구 질문**:
- 네 영역 중 어느 것을 조작하면 어떤 실패가 나타나는가?
- 성능 급락은 harness-off 조건에서 어떤 형태를 갖는가?
- 실패 분포는 harness-on 조건에서 어떻게 재분류되는가?

**핵심 구성** (네 영역 + 반례, 잠정적 — 실험 완료 후 재확인 필요):
- Inbound (E01-E04): model capability 조작 — harness가 받는 입력의 품질을 변경
- Outward (E05-E07): harness 설계 + contact surface 조작 — harness가 내보내는 형태를 변경
- Boundary (E08-E12): 실행 환경 제약 조작 — harness가 운영되는 자원·시간 제약을 변경
- Return (E13-E20): feedback loop 조작 — intervention에서 내재화까지의 스펙트럼
- 반례 (E21-E22): harness 방향과 무관한 조건 (task design 문제, compute saturation)
...
**상태**: 초고 있음 (정량 분석)
**핵심 메시지**: 실험실 metric → 운영 metric(MTTR, HER) → 비용 metric의 3단계 번역. Harness overhead의 최적점은 존재하며, 그 점 이상에서 harness는 새로운 1차 병목이 된다.

**탐구 질문**:
- 어떤 방향이 어떤 조건에서 1차 병목이었는가?
- 실패 분포는 harness 적용 후 어떻게 재분류되었는가?
- harness overhead 최적점은 어디에 있으며 비용 시나리오에 따라 어떻게 달라지는가?

**핵심 구성**:
1. 22개 실험 결과 종합: 방향별 병목 분포
2. 실패 재분류 정량 분석
3. 운영 metric 번역: MTTR과 Human Escalation Rate
4. 비용 metric 번역: harness overhead 최적점
...
- harness overhead × 성공률 최적점을 판단하는 방법을 이해한다.
...
**핵심 메시지**: Operational Compiler는 harness overhead × 성공률 pareto frontier를 따라 점진적으로 구성된다. §5에서 공개 실물 패턴(CLAUDE.md, AGENTS.md, GEMINI.md, .cursorrules) 비교를 통해 산업적 수렴을 확인한다.
...
2. Operational Compiler 설계 원칙 (harness overhead 관리 포함)
3. 점진적 업데이트 원칙: pareto frontier를 따라 이동하는 전략
4. Skill로 쓸 수 있는 능력의 극대화
5. **산업적 수렴: 공개 harness 패턴 비교** (CLAUDE.md, AGENTS.md, GEMINI.md, .cursorrules) + 4-component 분리 아키텍처와의 구조적 대응
...
**상태**: 초고 있음 (미해결 질문 정리)
**핵심 메시지**: Self-immune system = agent 내부의 자가 모니터링 + 성능 급락 근접 감지 + 자기 주도적 복구. Agent-1 → Agent-2 전환의 실질적 조건.

**탐구 질문**:
- agent 자가 모니터링의 재귀적 한계는 무엇인가?
- 어떤 AgentOps 기능이 harness 내재화가 가능하고 어떤 것은 불가능한가?
- Agent-1 → Agent-2 전환의 충분조건과 필요조건은 무엇인가?

**핵심 구성**:
1. 실험이 남긴 것 — 실패 재분류 재프레이밍
...
| Appendix | 파일 | 내용 |
|----------|------|------|
| A | `appendix-a-experiment-log-template.md` | 실험 로그 템플릿 (v4: 네 영역, 교차검증 포함) |

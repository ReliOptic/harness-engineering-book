...
Ch.7은 harness engineering과 AgentOps를 조작적으로 정의하고, **Failure Budget Reallocation(FBR)** 개념을 통해 harness의 효과를 측정하는 프레임을 제시한다. FBR의 핵심 관찰: "harness를 추가하면 실패가 줄어드는 게 아니라, 실패의 유형이 바뀐다." 이 관찰을 정량적으로 포착하는 프레임이 이 챕터의 독창적 기여다.

**이 대화의 목적**: FBR 개념을 정량화할 수 있는 측정 프레임을 설계한다. "좋은 재배분"과 "나쁜 재배분"을 구분하는 기준, harness overhead(HOR)와의 trade-off, 그리고 Ch.8 실험의 사전 등록 가설을 구체화한다.
...
2. **이 책의 기존 정의 (chapters/ch03-harness-and-agentops-defined.md)**:
   ...
   - HOR(Harness Overhead Ratio), MTTR(Mean Time To Resolution), HER(Human Escalation Rate)
...
### Failure Budget Reallocation (핵심 개념)
...
8. **HOR(Harness Overhead Ratio)의 최적점**: harness가 소비하는 token overhead가 전체 token budget의 몇 %일 때 가장 효율적인가요? overhead가 너무 낮으면 어떤 문제가 생기고, 너무 높으면 어떤 문제가 생기나요? 최적점을 찾는 방법을 제안해 보세요.
...
10. **Ch.8 실험 가설 사전 등록**: FBR, HOR, MTTR, HER를 사용하여 Ch.8의 22개 실험에서 검증할 핵심 가설 3개를 작성해 보세요. 각 가설은 "조건 X에서 변수 Y를 조작하면 지표 Z가 [방향]으로 변할 것이다" 형식으로 작성하세요.
...
3. **HOR 최적점 탐색 프레임**: 질문 8에서 도출된 overhead trade-off 분석을 Ch.9(실험 결과)에서 검증 가능한 형태로 정리
4. **실험 가설 목록**: 질문 10에서 작성한 가설을 Ch.7 §6(실험 프레임 사전 등록)에 직접 삽입할 수 있는 형태로 정리
5. **용어 정합성 점검**: 대화에서 사용된 harness/AgentOps/FBR/HOR/MTTR/HER의 정의가 glossary.md와 일치하는지 확인할 수 있는 대조표
